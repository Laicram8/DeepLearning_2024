...
def MiniBatchGD(X, Y, y, hyper, lamda, X_val, Y_val, y_val):
    ...
    for epoch in range(epochs):
        ...
        accuracy.append(ComputeAccuracy(X_val, y_val, W, b))
...
for i in range(len(W_trained)):
    acc_t= ComputeAccuracy(X_tst, y_tst, W_trained[i], b_trained[i])
    print(acc_t*100)
    #fig , ax = plot_acc(acc_t, colours, casename)
...
fig , ax = plot_acc(acc, colours, casename)
plt.savefig(path + 'val_t_3_batch_size.png')
fig , ax = plot_acc(acc_1, colours_1, casename_1)
plt.savefig(path + 'val_t_1_batch_size.png')