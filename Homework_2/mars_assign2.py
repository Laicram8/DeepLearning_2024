...
def LoadAll():
	...
	X = []; y = []
	for n in range(5):
		dt = unpickle(f"/home/mech/sanchis/sanchis/PHD_COURSES/Deep_learning/Assign_1/DirName/data/cifar-10-batches-py/data_batch_{n+1}")
...
def load_test_data():
    ...
    Returns: 
        ...
        Y	: [1,n]  
    """
    dt = unpickle("/home/mech/sanchis/sanchis/PHD_COURSES/Deep_learning/Assign_1/DirName/data/cifar-10-batches-py/test_batch")
    X = np.array(dt[b'data']).astype(np.float64)
...
def dataLoader_FullBatch(split_ratio=0.1):
    ...
    X, Y, X_val, Y_val = train_validation_split(X, Y.T, split_ratio)
    X_test,y, Y_test = load_test_data()
    print(f"FINISHED NORMALIZATION")
    print(f"SUMMARY DATA: TRAIN={X.T.shape},{Y.T.shape}")
    print(f"SUMMARY DATA: VAL={X_val.T.shape},{Y_val.T.shape}")
    print(f"SUMMARY DATA: TEST={X_test.shape},{Y_test.shape}")
    
    return X.T, Y.T, X_val.T, Y_val.T, X_test, Y_test
...
print('------------- GRID SEARCH ----------------')
batch = 100
lr_dict = {"n_s": 900, "eta_min": 1e-5, "eta_max": 1e-1} 
n_cycle = 3 
n_epochs = int(n_cycle * lr_dict['n_s'] * 2 / batch)
lamdas = {}
lamdas['search'] = []
...
for l in (range(n_search)):
    X_trn, Y_trn, X_val, Y_val, X_tst, Y_tst = dataLoader_FullBatch(split_ratio=0.02)  # Added missing closing parenthesis
    X_trn, X_tst, X_val = preprocess_data(X_trn, X_tst, X_val)
    train_dict = {'n_batch': batch, 'n_epochs': n_epochs}
    lamda = 10**(l_min + (l_max - l_min)*np.random.rand())
    lamdas['search'].append(lamda)
    h1 = 1e-5  # Given in assignment
    filename = name_case(**lr_dict, **train_dict, lamda=lamda)
    lr_sch = lr_scheduler(**lr_dict)
    W1, b1, W2, b2 = initialize_parameters(X_trn, Y_trn)
    ...
    P, h = EvaluateClassifier(X_tst, W1, b1, W2, b2)
    acc = compute_acc(X_tst, Y_tst, W1, b1, W2, b2, h, P)
    lamdas['acc'].append(acc)
    print(f"Acc = {acc*100}")
    
    # Assuming save_as_mat takes 7 arguments
    save_as_mat(W1, b1, W2, b2, hist, acc, "weights/" + filename)  # Added missing closing parenthesis
    fig, axs = plot_hist(hist, 10, batch)
    # Assuming axs and fig_dict are defined earlier
    for ax in axs:
        ax.set_xlabel('Update Step')
    fig.savefig(path + f'Figs/Loss_{filename}.jpg', **fig_dict)  # Added missing closing parenthesis
    #%%
print('------------ Best STAGE -------------')
X_trn, Y_trn, X_val, Y_val, X_tst, Y_tst = dataLoader_FullBatch(split_ratio=0.02)  # Added missing closing parenthesis
X_trn, X_tst, X_val = preprocess_data(X_trn, X_tst, X_val)

lr_dict = {"n_s": 1000, "eta_min": 1e-5, "eta_max": 1e-1} 
n_cycle = 3 
batch = 100
n_epochs = int(n_cycle * lr_dict['n_s'] * 2 / batch)
train_dict = {'n_batch': batch, 'n_epochs': n_epochs}
